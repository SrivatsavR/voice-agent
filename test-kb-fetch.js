...
// Run with: node test-kb-fetch.js
...
(async () => {
    // Override KB tool to return a deterministic answer
    ...
    // Walk through the flow to reach closure node
    ...
    await session.processTranscript('yes', tts); // GST (accept)
    ...
    const result = await session.processTranscript('What is the payment cycle for sellers?', tts);

    // Restore original tool
    searchKnowledgeBaseTool.execute = originalExecute;

    // Check that the spoken output contains the KB answer, not a hallucination.
